...
def stealSkill(target, stealTarget):
    if Player.DistanceTo(stealTarget) < 2:
...
def stealBigItem(bag, stealTarget):
    # level 1 big items
    for s in bag.Contains:
        if s.ItemID == 0x47E6:
            Player.HeadMessage(66, 'Dragon Egg')
            stealSkill(s, stealTarget)
            stealCheck()
...
while not Player.IsGhost:
    ...
    if stealTarget:
        ...
        Misc.Pause(50)
        stealBigItem(stealTargetBackpack, stealTarget)